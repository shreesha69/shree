...
const Contact = () => {
  ...
  return (
    <section className="relative py-20 px-4 max-w-6xl mx-auto overflow-auto">
      {/* Background */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
        {/* Contact Info */}
        <div className="bg-white dark:bg-gray-700 p-6 md:p-8 rounded-2xl shadow-lg flex flex-col gap-6 hover:scale-[1.02] transition-transform duration-300 w-full">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex items-center gap-3 text-gray-700 dark:text-gray-200">
                ...
                {c.link ? (
                  <a href={c.link} target="_blank" rel="noopener noreferrer" className="hover:underline break-words">{c.value}</a>
                ) : (
                  <span>{c.value}</span>
                )}
              </div>
        ...
        {/* Contact Form */}
        <form ref={formRef} onSubmit={submit} className="bg-white dark:bg-gray-700 p-6 md:p-8 rounded-2xl shadow-lg flex flex-col gap-4 hover:scale-[1.02] transition-transform duration-300 w-full">
          {/* Hidden time field */}
          <input type="hidden" name="time" value={new Date().toLocaleString()} />

          <input name="name" placeholder="Your Name" required className="w-full p-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-transparent focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-cyan-400 transition duration-300 shadow-sm" />
          <input name="email" placeholder="Your Email" type="email" required className="w-full p-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-transparent focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-cyan-400 transition duration-300 shadow-sm" />
          <input name="subject" placeholder="Subject" className="w-full p-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-transparent focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-cyan-400 transition duration-300 shadow-sm" />
          <textarea name="message" placeholder="Message" rows={5} className="w-full p-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-transparent focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-cyan-400 transition duration-300 shadow-sm"></textarea>
